 ...
 * - Timestamp
 * 
 * Útil para análisis de seguridad y detección de patrones anómalos.
 */
@Slf4j
@Component
public class AuditFilter extends OncePerRequestFilter {
    ...
    protected void doFilterInternal(HttpServletRequest request, HttpServletResponse response, 
        ...
        try {
            ...
            // Registrar solo endpoints que no sean health checks
    ...
    /**
     * Registra el evento de acceso en los logs.
     */
    private void auditLog(String clientIp, String userId, String method, String uri, int status, long duration) {
        if (status >= 200 && status < 300) {
            ...
            // Error del cliente - nivel WARN
            log.warn("AUDIT [{}] {} {} - User: {} - Status: {} - Duration: {}ms",
                    clientIp, method, uri, userId, status, duration);
        } else if (status >= 500) {
            // Error del servidor - nivel ERROR
