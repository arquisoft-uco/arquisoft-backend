...
| Reglas de dominio | `{Regla}Rule` (extiende `shared.rules.DomainRule<T>`) en `domain/{feature}/rules/` con su `{Regla}RuleImpl` en `rules/impl/`: POJO sin Spring, sin Lombok y **sin ninguna dependencia de constructor** — `validar(T)` es una decisión pura sobre un record de `domain/{feature}/model/`, no consulta nada, solo lanza. No son beans ni hace falta que lo sean: sin variabilidad ni ciclo de vida, no hay nada que un contenedor gestione. El `{Action}{Entity}ValidatorImpl` tiene un unico constructor sin argumentos que construye las reglas que ejecuta (`this.x = new XRuleImpl();`), sin `@RequiredArgsConstructor`. El `@Component` va solo en el validator |
...
| Literales en adapters | `ApiCodes` (códigos HTTP, en `shared:message/annotation/`), `{Contexto}ApiMessages` (textos Swagger), `FichasAuthorities` (authorities), `ApiSecurity.BEARER_AUTH` (esquema OpenAPI, en `shared:message/annotation/`) — nada quemado inline, salvo las rutas, que van inline como placeholder del yml |
