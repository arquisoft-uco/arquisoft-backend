...
class SolicitudNovedadAsesorRespondidaConsumerTest {
    ...
    private SolicitudNovedadAsesorRespondidaConsumer adapter;

    @BeforeEach
    void setUp() {
        adapter = new SolicitudNovedadAsesorRespondidaConsumer(
        ...
        lenient().when(enviarNotificacionInteractor.ejecutar(any()))
                .thenReturn(new EnvioNotificacionResult.Enviada("evt", "ana.est@soyuco.edu.co"));
    }
    ...
    void debeConfirmarElMensaje_cuandoElProcesamientoTermina() throws Exception {
        // Act
        adapter.onSolicitudNovedadAsesorRespondida(mensajeCon(UUID.randomUUID().toString(), 1L), channel);
    ...
    void debeEnviarNackSinReencolar_cuandoElInteractorFalla() throws Exception {
        ...
        // Act
        adapter.onSolicitudNovedadAsesorRespondida(mensajeCon(UUID.randomUUID().toString(), 2L), channel);
    ...
    void debeReencolarSinNotificar_cuandoLaPlantillaNoTieneTexto() throws Exception {
        ...
        try {
            // Act
            adapter.onSolicitudNovedadAsesorRespondida(
