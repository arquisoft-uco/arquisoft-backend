...
class SolicitudCambioAsesorEnviadaConsumerTest {
    ...
    private SolicitudCambioAsesorEnviadaConsumer adapter;

    @BeforeEach
    void setUp() {
        adapter = new SolicitudCambioAsesorEnviadaConsumer(
        ...
        String payloadJson = """
                {
                    ...
                    "mensajeSolicitud": "Necesito cambiar de asesor por incompatibilidad de horarios",
    ...
    void debeTraducirElEventoEnUnaNotificacion_cuandoElPayloadEsValido() throws Exception {
        ...
        // Act
        adapter.onSolicitudCambioAsesorEnviada(mensajeCon(idEvento, 1L), channel);
        ...
        assertThat(command.tipo()).isEqualTo(TipoNotificacion.SOLICITUD_CAMBIO_ASESOR);
    ...
    void debeResolverElAsuntoYElCuerpoDesdeElCatalogo_cuandoConstruyeElComando() throws Exception {
        // Act
        adapter.onSolicitudCambioAsesorEnviada(mensajeCon(UUID.randomUUID().toString(), 1L), channel);
        ...
        assertThat(captor.getValue().cuerpo())
                ...
                .contains("Necesito cambiar de asesor por incompatibilidad de horarios");
    ...
    void debeEnviarNackSinReencolar_cuandoElInteractorFalla() throws Exception {
        ...
        // Act
        adapter.onSolicitudCambioAsesorEnviada(mensajeCon(UUID.randomUUID().toString(), 2L), channel);
    ...
    void debeReencolarSinNotificar_cuandoLaPlantillaNoTieneTexto() throws Exception {
        ...
        try {
            // Act
            adapter.onSolicitudCambioAsesorEnviada(mensajeCon(UUID.randomUUID().toString(), 3L), channel);
