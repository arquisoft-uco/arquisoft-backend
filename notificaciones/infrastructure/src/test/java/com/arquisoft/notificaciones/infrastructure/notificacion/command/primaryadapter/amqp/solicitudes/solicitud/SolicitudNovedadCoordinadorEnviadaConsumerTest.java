...
class SolicitudNovedadCoordinadorEnviadaConsumerTest {
    ...
    private SolicitudNovedadCoordinadorEnviadaConsumer adapter;

    @BeforeEach
    void setUp() {
        adapter = new SolicitudNovedadCoordinadorEnviadaConsumer(
        ...
        String payloadJson = """
                {
                    ...
                    "mensajeSolicitud": "No puedo asistir a la reunion del jueves",
    ...
    void debeTraducirElEventoEnUnaNotificacion_cuandoElPayloadEsValido() throws Exception {
        ...
        // Act
        adapter.onSolicitudNovedadCoordinadorEnviada(mensajeCon(idEvento, 1L), channel);
        ...
        assertThat(command.tipo()).isEqualTo(TipoNotificacion.SOLICITUD_NOVEDAD_COORDINADOR);
    ...
    void debeResolverElAsuntoYElCuerpoDesdeElCatalogo_cuandoConstruyeElComando() throws Exception {
        // Act
        adapter.onSolicitudNovedadCoordinadorEnviada(mensajeCon(UUID.randomUUID().toString(), 1L), channel);
        ...
        assertThat(captor.getValue().cuerpo())
                ...
                .contains("No puedo asistir a la reunion del jueves");
    ...
    void debeEnviarNackSinReencolar_cuandoElInteractorFalla() throws Exception {
        ...
        // Act
        adapter.onSolicitudNovedadCoordinadorEnviada(mensajeCon(UUID.randomUUID().toString(), 2L), channel);
    ...
    void debeReencolarSinNotificar_cuandoLaPlantillaNoTieneTexto() throws Exception {
        ...
        try {
            // Act
            adapter.onSolicitudNovedadCoordinadorEnviada(mensajeCon(UUID.randomUUID().toString(), 3L), channel);
