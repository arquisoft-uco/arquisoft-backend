...
class RevisionItemAgregadoConsumerTest {
    ...
    private RevisionItemAgregadoConsumer adapter;

    @BeforeEach
    void setUp() {
        adapter = new RevisionItemAgregadoConsumer(
    ...
    void debeNotificarACadaEstudiante_cuandoElPayloadTraeVarios() throws Exception {
        ...
        // Act
        adapter.onRevisionItemAgregado(mensajeCon(idEvento, DOS_ESTUDIANTES, 1L), channel);
    ...
    @Test
    void debeUsarElTipoDeRevisionItemAgregado_cuandoSeAbreElAbanico() throws Exception {
        // Act
        adapter.onRevisionItemAgregado(
                mensajeCon(UUID.randomUUID().toString(), DOS_ESTUDIANTES, 1L), channel);

        // Assert
        assertThat(comandosEmitidos(2))
                .extracting(EnviarNotificacionCommand::tipo)
                .containsOnly(TipoNotificacion.REVISION_ITEM_AGREGADO);
    }

    @Test
    void debePersonalizarElCuerpoConElNombreDeCadaEstudiante_cuandoSeAbreElAbanico() throws Exception {
        // Act
        adapter.onRevisionItemAgregado(
                mensajeCon(UUID.randomUUID().toString(), DOS_ESTUDIANTES, 1L), channel);
    ...
    void debeNoEnviarNada_cuandoElEventoLlegaSinEstudiantes() throws Exception {
        // Act
        adapter.onRevisionItemAgregado(mensajeCon(UUID.randomUUID().toString(), "[]", 2L), channel);
    ...
    void debeNoEnviarNada_cuandoElCampoDeEstudiantesLlegaNulo() throws Exception {
        // Act
        adapter.onRevisionItemAgregado(mensajeCon(UUID.randomUUID().toString(), "null", 3L), channel);
