...
class SolicitudNovedadAsesorEnviadaConsumerTest {
    ...
    private SolicitudNovedadAsesorEnviadaConsumer adapter;

    @BeforeEach
    void setUp() {
        adapter = new SolicitudNovedadAsesorEnviadaConsumer(
        ...
        lenient().when(enviarNotificacionInteractor.ejecutar(any()))
                .thenReturn(new EnvioNotificacionResult.Enviada("evt", "pedro.asesor@soyuco.edu.co"));
    }
        ...
        String payloadJson = """
                {
                    ...
                    "destinatarioNombre": "Pedro Asesor",
                    "destinatarioEmail": "pedro.asesor@soyuco.edu.co"
                }
    ...
    void debeTraducirElEventoEnUnaNotificacion_cuandoElPayloadEsValido() throws Exception {
        ...
        // Act
        adapter.onSolicitudNovedadAsesorEnviada(mensajeCon(idEvento, 1L), channel);
        ...
        assertThat(command.tipo()).isEqualTo(TipoNotificacion.SOLICITUD_NOVEDAD_ASESOR);
        assertThat(command.destinatarioNombre()).isEqualTo("Pedro Asesor");
        assertThat(command.destinatarioEmail()).isEqualTo("pedro.asesor@soyuco.edu.co");
    }

    @Test
    void debeResolverElAsuntoYElCuerpoDesdeElCatalogo_cuandoConstruyeElComando() throws Exception {
        // Act
        adapter.onSolicitudNovedadAsesorEnviada(mensajeCon(UUID.randomUUID().toString(), 1L), channel);
    ...
    void debeEnviarNackSinReencolar_cuandoElInteractorFalla() throws Exception {
        ...
        // Act
        adapter.onSolicitudNovedadAsesorEnviada(mensajeCon(UUID.randomUUID().toString(), 2L), channel);
    ...
    void debeReencolarSinNotificar_cuandoLaPlantillaNoTieneTexto() throws Exception {
        ...
        try {
            // Act
            adapter.onSolicitudNovedadAsesorEnviada(mensajeCon(UUID.randomUUID().toString(), 3L), channel);
