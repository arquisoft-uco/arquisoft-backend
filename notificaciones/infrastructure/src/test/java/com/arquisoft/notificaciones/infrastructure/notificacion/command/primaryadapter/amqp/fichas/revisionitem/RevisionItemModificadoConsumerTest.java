...
class RevisionItemModificadoConsumerTest {
    ...
    private RevisionItemModificadoConsumer adapter;

    @BeforeEach
    void setUp() {
        adapter = new RevisionItemModificadoConsumer(
        ...
        String payloadJson = """
                {
                    "idEvento": "%s",
                    "itemId": "%s",
                    "estadoRevisionNombre": "Visualizada",
    ...
    void debeNotificarACadaEstudiante_cuandoElPayloadTraeVarios() throws Exception {
        ...
        // Act
        adapter.onRevisionItemModificado(mensajeCon(idEvento, DOS_ESTUDIANTES, 1L), channel);
    ...
    @Test
    void debeUsarElTipoDeRevisionItemModificado_cuandoSeAbreElAbanico() throws Exception {
        // Act
        adapter.onRevisionItemModificado(
                mensajeCon(UUID.randomUUID().toString(), DOS_ESTUDIANTES, 1L), channel);

        // Assert
        assertThat(comandosEmitidos(2))
                .extracting(EnviarNotificacionCommand::tipo)
                .containsOnly(TipoNotificacion.REVISION_ITEM_MODIFICADO);
    }

    @Test
    void debePersonalizarElCuerpoConElNombreYElEstado_cuandoSeAbreElAbanico() throws Exception {
        // Act
        adapter.onRevisionItemModificado(
                mensajeCon(UUID.randomUUID().toString(), DOS_ESTUDIANTES, 1L), channel);
        ...
        assertThat(comandos.get(0).cuerpo()).contains("Ana Gomez", "Sistema de gestión", "Visualizada");
        assertThat(comandos.get(1).cuerpo()).contains("Luis Diaz", "Sistema de gestión", "Visualizada");
    }

    @Test
    void debeNoEnviarNada_cuandoElEventoLlegaSinEstudiantes() throws Exception {
        // Act
        adapter.onRevisionItemModificado(mensajeCon(UUID.randomUUID().toString(), "[]", 2L), channel);
    ...
    void debeNoEnviarNada_cuandoElCampoDeEstudiantesLlegaNulo() throws Exception {
        // Act
        adapter.onRevisionItemModificado(mensajeCon(UUID.randomUUID().toString(), "null", 3L), channel);
