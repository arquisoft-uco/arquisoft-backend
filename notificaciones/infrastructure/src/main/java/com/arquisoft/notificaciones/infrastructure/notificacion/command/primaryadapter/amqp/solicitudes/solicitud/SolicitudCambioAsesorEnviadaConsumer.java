...
@Component
public class SolicitudCambioAsesorEnviadaConsumer extends AbstractNotificacionConsumer {

    private final EnviarNotificacionInteractor enviarNotificacionInteractor;

    public SolicitudCambioAsesorEnviadaConsumer(
    ...
    @RabbitListener(queues = NotificacionesSolicitudesQueueConfig.CAMBIO_ASESOR_QUEUE)
    public void onSolicitudCambioAsesorEnviada(Message message, Channel channel) throws IOException {
        withCorrelation(message, channel, () -> {
            SolicitudCambioAsesorEnviadaPayload payload =
                    deserialize(message, SolicitudCambioAsesorEnviadaPayload.class);

            logger.info(
                    ConsumidorKey.LOG_SOLICITUD_CAMBIO_ASESOR_RECIBIDO,
                    payload.solicitudId(),
            ...
            registrar(enviarNotificacionInteractor.ejecutar(EnviarNotificacionCommand.crear(
                    payload.idEvento(),
                    TipoNotificacionEvento.SOLICITUD_CAMBIO_ASESOR.getCodigo(),
                    payload.destinatarioNombre(),
                    payload.destinatarioEmail(),
                    plantilla(
                            PlantillaKey.ASUNTO_SOLICITUD_CAMBIO_ASESOR,
                            payload.remitenteNombre()),
                    plantilla(
                            PlantillaKey.CUERPO_SOLICITUD_CAMBIO_ASESOR,
                            payload.destinatarioNombre(),
                            payload.remitenteNombre(),
