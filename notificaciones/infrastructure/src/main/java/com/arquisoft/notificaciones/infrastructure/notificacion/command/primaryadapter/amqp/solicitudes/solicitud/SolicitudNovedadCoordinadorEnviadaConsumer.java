...
@Component
public class SolicitudNovedadCoordinadorEnviadaConsumer extends AbstractNotificacionConsumer {

    private final EnviarNotificacionInteractor enviarNotificacionInteractor;

    public SolicitudNovedadCoordinadorEnviadaConsumer(
    ...
    @RabbitListener(queues = NotificacionesSolicitudesQueueConfig.NOVEDAD_COORDINADOR_QUEUE)
    public void onSolicitudNovedadCoordinadorEnviada(Message message, Channel channel) throws IOException {
        withCorrelation(message, channel, () -> {
            SolicitudNovedadCoordinadorEnviadaPayload payload =
                    deserialize(message, SolicitudNovedadCoordinadorEnviadaPayload.class);

            logger.info(
                    ConsumidorKey.LOG_SOLICITUD_NOVEDAD_COORDINADOR_RECIBIDO,
                    payload.solicitudId(),
            ...
            registrar(enviarNotificacionInteractor.ejecutar(EnviarNotificacionCommand.crear(
                    payload.idEvento(),
                    TipoNotificacionEvento.SOLICITUD_NOVEDAD_COORDINADOR.getCodigo(),
                    payload.destinatarioNombre(),
                    payload.destinatarioEmail(),
                    plantilla(
                            PlantillaKey.ASUNTO_SOLICITUD_NOVEDAD_COORDINADOR,
                            payload.remitenteNombre()),
                    plantilla(
                            PlantillaKey.CUERPO_SOLICITUD_NOVEDAD_COORDINADOR,
                            payload.destinatarioNombre(),
                            payload.remitenteNombre(),
