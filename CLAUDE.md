...
Hexagonal Architecture (Ports & Adapters) with **9 bounded contexts** and **15 shared modules**. Contexts communicate exclusively via RabbitMQ domain events — they never import each other.
...
`shared:util`, `shared:exception`, `shared:validation`, `shared:domain`, `shared:application`, `shared:query`, `shared:logger`, `shared:tracing`, `shared:redis`, `shared:amqp`, `shared:web`, `shared:minio`, `shared:jpa`, `shared:message`, `shared:notification`
