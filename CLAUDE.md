...
**Where the conversion happens:** the **use case** maps `Domain → Entity` before calling the port, and the **finder** maps `Entity → Domain` after it returns. Adapters no longer translate anything — they delegate to the repository and nothing else. A mapper needing a `@ManyToOne` association builds it as an id-only detached instance (`AsesorFichaEntity.builder().id(...).build()`); with no cascade configured, Hibernate writes just the foreign key, which is what replaces the old `getReferenceById` proxy the adapter used to resolve. Enums are the mapper's job too: `EstadoFicha.desde(entity.getEstadoFicha().getId())` validates the id against the catalog and throws `EstadoFichaNoEncontradoException` (422) directly on an unrecognized or blank value — the enum owns this self-validation the same way an aggregate validates its own invariants, so a mapper can surface a proper domain error instead of letting a bare `IllegalArgumentException` from `valueOf` surface as a 500.
