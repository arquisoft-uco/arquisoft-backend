...
**Use case implementations:** `{Action}{Entity}UseCaseImpl` in `usecase/impl/`, implementing `{Action}{Entity}UseCase` (e.g., `RegistrarFichaPerfilUseCaseImpl`, `AuthenticateUserUseCaseImpl`). Annotated `@Component` — **never `@Service`**, which is not used anywhere in this project. The use case is a plain collaborator invoked by its interactor — the adapter always injects the `Interactor`, never the `UseCase`.
...
**Message catalog:** runtime texts (errors, logs) live in `.properties` under `shared:message` and resolve through the injectable `MessageCatalog` port — `private final MessageCatalog catalog;` in application/infrastructure, the static `Messages` facade in domain (aggregates and exceptions have no injection point). Keys follow `contexto.capa.objeto.tipo.descripcion`. What the compiler forces to stay a constant — error codes (`*Codes`), limits (`*Limits`), field names (`*Fields`) — does not go to the bundle, because annotation values must be constant expressions (JLS §9.7.1). `MessageCatalogClavesTest` fails the build on a key with no text or a text with no key. See [shared/message/README.md](shared/message/README.md).
