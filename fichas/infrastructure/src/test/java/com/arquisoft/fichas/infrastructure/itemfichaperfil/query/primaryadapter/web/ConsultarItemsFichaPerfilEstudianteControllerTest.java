...
@WebMvcTest(ConsultarItemsFichaPerfilEstudianteController.class)
@Import({AppLoggerConfig.class, GlobalAppExceptionHandler.class, TrazabilidadConfig.class,
        ConsultarItemsFichaPerfilEstudianteControllerTest.TestSecurityConfig.class})
class ConsultarItemsFichaPerfilEstudianteControllerTest {
    ...
    @MockitoBean
    private ConsultarItemsFichaPerfilEstudianteInteractor consultarItemsFichaPerfilEstudianteInteractor;
    ...
    void debeRetornar200ConLista_cuandoHayItems() throws Exception {
        ...
        when(consultarItemsFichaPerfilEstudianteInteractor.ejecutar(any(ConsultarItemsFichaPerfilEstudianteQuery.class)))
                .thenReturn(List.of(new ItemFichaPerfilReadModel(
                        itemId, FICHA_ID, "OBJETIVO_GENERAL", "Objetivo General", "Contenido")));

        // Act & Assert
        mockMvc.perform(get("/fichas-perfil/{fichaPerfilId}/items/estudiante", FICHA_ID)
                        .with(jwtConRol(FichasAuthorities.ITEM_FICHA_PERFIL_ESTUDIANTE_VIEW)))
    ...
    void debeRetornar400_cuandoFichaPerfilIdNoEsUuid() throws Exception {
        // Act & Assert
        mockMvc.perform(get("/fichas-perfil/{fichaPerfilId}/items/estudiante", "no-es-uuid")
                        .with(jwtConRol(FichasAuthorities.ITEM_FICHA_PERFIL_ESTUDIANTE_VIEW)))
    ...
    @Test
    void debePasarSubjectDelJwtComoEstudiante() throws Exception {
        // Arrange
        when(consultarItemsFichaPerfilEstudianteInteractor.ejecutar(any(ConsultarItemsFichaPerfilEstudianteQuery.class)))
                .thenReturn(List.of());
        ...
        // Assert
        var captor = ArgumentCaptor.forClass(ConsultarItemsFichaPerfilEstudianteQuery.class);
        verify(consultarItemsFichaPerfilEstudianteInteractor).ejecutar(captor.capture());
