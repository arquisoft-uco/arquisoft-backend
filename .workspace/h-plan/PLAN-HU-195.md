...
| Refactor (post-reporte) | Coordinador | ✅ Completado | 2026-08-28 | El usuario señaló que `AgregarRevisionItemUseCaseImpl` convertía el conteo del `RevisionesDelItemFinder` a `boolean` (`> 0`) antes de llegar al validator/Rule — comparación de negocio hecha fuera del Rule. Se corrigió siguiendo el precedente exacto de `EstudianteFichaPerfilCupoDisponibleRuleImpl`/`CupoEstudiantesFicha` (el valor numérico crudo viaja hasta el Rule, que hace la comparación). Cambios: `DisponibilidadRevisionItem` pasa de `boolean yaExiste` a `long cantidadRevisiones`; `RevisionItemNoDuplicadaRuleImpl` ahora compara `cantidadRevisiones() > 0`; `AgregarRevisionItemValidator`/`Impl` y `AgregarRevisionItemUseCaseImpl` propagan el `long` sin convertirlo a `boolean`. Se actualizaron `RevisionItemNoDuplicadaRuleImplTest`, `AgregarRevisionItemValidatorTest` y `AgregarRevisionItemUseCaseTest` (mismos casos, `long` en vez de `boolean`). Verificado con `--rerun-tasks`: `fichas:domain`/`fichas:application` compilan, pasan Checkstyle, tests y `jacocoTestCoverageVerification` sin regresiones; `fichas:infrastructure` no se vio afectado (compila igual). |
