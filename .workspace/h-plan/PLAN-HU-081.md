...
- El transporte es una **consulta síncrona vía `shared:web-client`** (módulo que aún no existe — dueño: HT de web-client). Ver CLAUDE.md § *Consultas síncronas entre contextos*.

Mientras tanto, `AsignacionProyectoOutputAdapter` (`infrastructure/asignacionproyecto/command/secondaryadapter/webclient/`) es un **stub**: devuelve `true` y loguea `warn`. Consecuencia: **RN-7 no rechaza nada todavía**. Registrado en CLAUDE.md § *Desviaciones conocidas*.
...
4. Quitar la fila del stub en CLAUDE.md § *Desviaciones conocidas*.
...
|---|---|---|---|---|
| Emite | `eventPublisher.publish(...)` (puerto `EventPublisher` de `shared:application`) → `SpringModulithEventPublisher` (`@Primary`) → outbox `event_publication` en la tx del interactor → `ModulithAmqpExternalizationConfig` externaliza al exchange `arquisoft.events` | `EventTopics.Solicitudes.NOVEDAD_COORDINADOR_ENVIADA` = `solicitudes.solicitud.novedad_coordinador_enviada` (= `getTemaEvento()`) | `SolicitudNovedadCoordinadorEnviadaEvent` (solicitudId, remitenteNombre, destinatarioNombre, destinatarioEmail, mensajeSolicitud) | `notificaciones` → `SolicitudNovedadCoordinadorEnviadaConsumer` (correo al coordinador) |

**Ya NO es una desviación.** HU081 usa el patrón único de `arquisoft-arquitectura`, idéntico a `CambiarAsesorFichaUseCaseImpl` → `AsesorFichaCambiadoEvent`.

**Actualización 2026-09-05 (revierte "solo lado publicación"):** ahora hay consumidor. El evento se enriqueció con `remitenteNombre`/`destinatarioNombre`/`destinatarioEmail` (leídos de la réplica local `usuario` en el use case, vía `DatosUsuarioFinder` + `UsuarioOutputPort.buscarPorId`) y se movió la routing key a `EventTopics.Solicitudes`. En `notificaciones` se agregaron las 8 piezas del patrón "Transición de estado ⇒ notificación" (`arquisoft-arquitectura`): `NotificacionesSolicitudesQueueConfig` (cola + `.dead` + bindings vía `ColaEvento.declarar`), `SolicitudNovedadCoordinadorEnviada{Payload,Consumer}` en `.../amqp/solicitudes/solicitud/`, `TipoNotificacion.SOLICITUD_NOVEDAD_COORDINADOR` (+ espejo `TipoNotificacionEvento`), `PlantillaKey.ASUNTO_/CUERPO_SOLICITUD_NOVEDAD_COORDINADOR` + `ConsumidorKey.LOG_*` + textos en `catalogo/notificaciones.properties`. Sin migración (`TipoNotificacion` es VARCHAR), sin client role nuevo. Validado de punta a punta con Mailpit. Referencia copiada: `AsesorFichaCambiadoConsumer`.
...
| Añadido RN-7 (validación de asignación destinatario↔estudiante) | usuario + Claude | ✅ Completado — `BUILD SUCCESSFUL` | 2026-09-05 | Decidido con Brayan (transcripción de reunión): la validación "el destinatario es el coordinador/asesor asignado" se hace con **consulta síncrona a `proyectos` vía `shared:web-client`**, no con réplica por eventos. Alcance de este cambio: **solo lado `solicitudes`, solo HU-081**. `shared:web-client` lo crea Brayan+Juan aparte. Nuevos: `ExistenciaAsignacionResponsable`/`ConsultaAsignacionResponsable` (domain model), `DestinatarioAsignadoRule`(+impl), `DestinatarioNoAsignadoException` (422), `AsignacionProyectoOutputPort` (application, feature `asignacionproyecto`), `DestinatarioAsignadoFinder`(+impl), `AsignacionProyectoOutputAdapter` (**STUB** — devuelve `true`, `warn`). Modificados: `EnviarSolicitudNovedadCoordinadorValidator`(+impl) `+validarAsignacionDestinatario`, `EnviarSolicitudNovedadCoordinadorUseCaseImpl` (inyecta el finder, llama tras `validarExistenciaUsuarios`), `SolicitudKey` (+2 claves), `SolicitudesCodes.Solicitud` (+`DESTINATARIO_NO_ASIGNADO`), `catalogo/solicitudes.properties` (+2). Tests: `DestinatarioAsignadoRuleImplTest`, `DestinatarioAsignadoFinderImplTest`, `AsignacionProyectoOutputAdapterTest` (nuevos); `EnviarSolicitudNovedadCoordinadorValidatorImplTest` + `...UseCaseImplTest` (actualizados). CLAUDE.md: nueva § *Consultas síncronas entre contextos*, excepción a "never import each other", carve-out de "single consumer", fila en *Known deviations*. RN-7 §3.1 con checklist de activación. Catálogo cargado en Redis remoto (`172.16.1.12`, 273 claves, `solicitudes` 9→11). Build verde: tests + checkstyle + jacoco. |
| Commit RN-7 + merge develop + push | usuario + Claude | ✅ Hecho | 2026-09-05 | `79f187ac` feat(solicitudes) RN-7 · `449c50d0` merge de `develop` (8 commits: #93–#100). Reconciliado #94 (AppLogger→ClaveMensaje: migrados ~13 call sites de `solicitudes`) y CLAUDE.md (reescrito por develop, re-aplicadas las 3 adiciones de RN-7 en español). Build verde. Pushed a `origin/feature/HU-081`. **Pendiente aparte:** (a) actualizar `arquisoft-arquitectura` + `1-planificador` + `4a-validator` con el patrón "Consultas síncronas entre contextos"; (b) notificaciones — correo al coordinador al enviar la solicitud. |
| Enriquecer evento + correo al coordinador (Parte A + B) | usuario + Claude | ✅ Completado — build verde + validado con Mailpit | 2026-09-05 | **Parte A (`solicitudes`):** `SolicitudNovedadCoordinadorEnviadaEvent` pasa de 6 a 5 campos (`solicitudId, remitenteNombre, destinatarioNombre, destinatarioEmail, mensajeSolicitud`) — se sacaron `remitenteUsuarioId`/`destinatarioUsuarioId`/`fechaCreacion`/`tipoSolicitud` (traza especulativa; `idEvento`/`ocurridoEn` de la base + el tipo lo dice el evento). Routing key → `EventTopics.Solicitudes.NOVEDAD_COORDINADOR_ENVIADA`. `UsuarioExisteFinder` → **`DatosUsuarioFinder`** (`Optional<UsuarioEntity>`); `UsuarioOutputPort` gana `buscarPorId` (el `existePorId` se queda para el upsert del consumer). El use case lee las 2 réplicas, deriva existencia con `.isPresent()`, y usa nombre/email para el evento. **Parte B (`notificaciones`):** las 8 piezas — `NotificacionesSolicitudesQueueConfig`, `SolicitudNovedadCoordinadorEnviada{Payload,Consumer}` (`amqp/solicitudes/solicitud/`), `TipoNotificacion`/`TipoNotificacionEvento` +`SOLICITUD_NOVEDAD_COORDINADOR`, `PlantillaKey` +ASUNTO/CUERPO, `ConsumidorKey` +LOG, `catalogo/notificaciones.properties` +3 (ASCII). Consumer copia de `AsesorFichaCambiadoConsumer` (con `logger.info(ClaveMensaje,…)` de #94 en vez de `Mensajes.obtener`). Tests: `SolicitudNovedadCoordinadorEnviadaConsumerTest`, `DatosUsuarioFinderImplTest` (nuevos); use case + adapter tests actualizados. Catálogo recargado en Redis remoto (281 claves). **Sin migración, sin client role.** Pendiente: commit (Parte A + B) + publicar plan/validator en `arquisoft-docs`. |
| Validación | @validator-analyze | ✅ **APROBADO** — Score ≈95/100, 0 bloqueantes | 2026-09-05 | **Ronda 8 (RN-7 + Parte A + Parte B).** Enriquecimiento del evento `SolicitudNovedadCoordinadorEnviadaEvent` (6→5 campos), `DatosUsuarioFinder`, y las 8 piezas del patrón "transición de estado ⇒ notificación" en `notificaciones` (correo al coordinador). 2 menores corregidos por el implementador post-análisis: `[2.6]` `DatosUsuarioFinder` ahora devuelve `Optional<UsuarioDomain>` (mapea `Entity→Domain` con `UsuarioMapper::toDomain`); `[2.12]` `logger.info(LOG_ENVIADA)` movido tras `eventPublisher.publish(...)`. 2 menores remanentes sin acción (`SolicitudNovedadCoordinadorEnviadaPayloadTest` ausente — repo inconsistente; `LOG_ASIGNACION_NO_VERIFICADA` en enum `SolicitudKey`/segmento `solicitud` — pragmático para stub de 1 línea). O-1 (patrón consulta síncrona en skill) RESUELTA. O-2 (`AsignacionProyectoOutputAdapter` STUB) documentada. Gate del usuario verde: `test` + `checkstyleMain`/`checkstyleTest` + jacoco + validado end-to-end con Mailpit (POST → 201 → cola → consumer → correo renderizado). |
| Reporte | @validator-report | ✅ Persistido (ronda 8) | 2026-09-05 | `.workspace/validator/validator-HU-081.md` — sobrescrito con el análisis de ronda 8 (RN-7 + Parte A + Parte B). Estado APROBADO · Score ≈95/100 · 0 bloqueantes. |
| Commit + publicación docs (RN-7 + Parte A/B) | usuario + Claude | ✅ Hecho | 2026-09-05 | Rama `feature/HU-081`: `6975e684` feat(solicitudes) enriquecer evento · `ec517f62` feat(notificaciones) correo al coordinador · `44894fe9` docs(agentes) consultas síncronas entre contextos · `056054f5` docs(plan). Pushed `449c50d0..056054f5`. Publicado en `arquisoft-docs/main`: `docs/hus/planes/PLAN-HU-081.md` + `docs/hus/validaciones/VALIDATOR-HU-081.md`. |
| PR hacia develop | — | ⏳ Pendiente | | La rama acumula: HU-081 base + RN-7 + Parte A/B. Abrir PR cuando el equipo decida el merge. |
