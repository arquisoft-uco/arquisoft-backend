...
**Orden de validación:** 1) integridad del dato (`Command.crear` + `SolicitudDomain.crear`) → 2) existencia contra BD (RN-3, RN-4 primero; RN-5 después) → 3) reglas del agregado (ninguna extra). La regla de duplicidad confía en que las de existencia ya lanzaron.
