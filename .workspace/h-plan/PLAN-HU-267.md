...
- **¿Usa AggregateRoot?:** No. Es un CRUD interno sin consumidores en otros contextos; no emite eventos de dominio.
...
- **Código de referencia verificado:** `fichas/fichaperfil`, `shared:domain`, `shared:message` y configuraciones de DataSource por contexto.
...
- **Observaciones del usuario:** se aprobaron nombre único sin distinguir mayúsculas/minúsculas, respuesta UUID, ruta bajo `/evaluaciones`, client role exclusivo de administrador y alcance limitado al registro. Posteriormente se decidió no publicar eventos porque ningún otro contexto consume esta operación.

---

## 1. Resumen Funcional

La HU permite que un administrador registre un nuevo ítem cualitativo que los jurados podrán utilizar posteriormente en sus evaluaciones. El recurso contiene un nombre institucional único y una descripción; el sistema genera su UUID y lo persiste atómicamente.
...
| 2 | Nombre o descripción están vacíos o exceden 100/300 caracteres | `400 Bad Request` con `fieldErrors`, sin consultar la BD |
...
| 4 | La persistencia termina correctamente | La fila queda en `item_cualitativo_jurado` dentro de la transacción |
| 5 | La persistencia falla | Se revierte el registro del ítem |
...
- **Tipo:** entidad de dominio plana; no extiende `AggregateRoot` porque no emite eventos.
...
- **Creación:** genera UUID con `UtilUUID`, aplica trim con `UtilTexto` y valida mediante Notification Pattern.
- **Reconstrucción:** restaura los datos persistidos sin validar ni generar UUID.
...
Esta HU no publica eventos de dominio porque ningún otro contexto consume el registro del ítem.
...
| Componente | Uso |
|---|---|
| PostgreSQL `evaluaciones` | Persistencia del catálogo |
...
|---|---|---|
| domain | `evaluaciones/domain/src/main/java/com/arquisoft/evaluaciones/domain/itemcualitativojurado/ItemCualitativoJuradoDomain.java` | Entidad, factories e integridad |
...
| application | `evaluaciones/application/src/main/java/com/arquisoft/evaluaciones/application/itemcualitativojurado/command/usecase/impl/RegistrarItemCualitativoJuradoUseCaseImpl.java` | Unicidad, persistencia y log |
...
- `ItemCualitativoJuradoDomain.crear(nombre, descripcion)` valida con `ValidationResult`, genera el UUID y asigna los valores normalizados.
- `ItemCualitativoJuradoDomain.reconstruir(id, nombre, descripcion)` no valida ni genera UUID.
...
Esta HU no publica ni consume eventos. `RegistrarItemCualitativoJuradoInteractorImpl` conserva la
transacción de `evaluaciones`, dentro de la cual el UseCase valida y persiste el Entity. No se inyecta
`EventPublisher`, no se crea outbox y no se externaliza nada a RabbitMQ.

---

## 11. Migraciones de Base de Datos

La HU introduce una única migración en `db/migration/evaluaciones`.

### `V20260825121912__crear_item_cualitativo_jurado.sql`
...
`evaluacionesFlyway` ejecuta esta migración antes de construir el EntityManagerFactory.
...
- `debeReconstruirSinValidar_cuandoSeCargaDesdePersistencia`.
...
- Mapper crea el Domain.
- Finder devuelve exactamente el booleano del OutputPort y nunca lanza por `false`.
- Validator delega en la Rule.
- UseCase exitoso: consulta unicidad, persiste Entity, registra log y retorna UUID.
- Nombre duplicado: no persiste ni registra el log exitoso.
- Fallo de persistencia: detiene el flujo y no registra el log exitoso.
...
- [ ] Entidad de dominio plana, sin `AggregateRoot` ni eventos.
...
- [ ] UseCase persiste sin inyectar ni invocar `EventPublisher`.
...
- [ ] Una migración timestamp para `item_cualitativo_jurado`; sin tabla `event_publication`.
