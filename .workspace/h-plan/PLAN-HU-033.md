...
| 3 | El sistema valida que el estudiante sea propietario de la ficha | Si el estudiante no es propietario, devuelve 400 con `ItemFichaNoPropiaException` (extiende `ApplicationException`, regla arquitectónica del proyecto) |
| 4 | El sistema valida que el contenido sea válido | Contenido obligatorio (no vacío), máximo 7000 caracteres |
| 5 | Los campos `tipo_item_id` y `ficha_perfil_id` son inmutables | No se modifican aunque el cliente los envíe en el request |
| 6 | El sistema valida que la ficha esté en un estado modificable | Si la ficha está en un estado que impide modificaciones, devuelve 400 con excepción apropiada |
...
- La ficha debe estar en un estado que permita modificaciones (revisar implementación de HU-161/HU-162 para el patrón de validación de estado).
...
|---------------|-----------------|
| `fichas/domain/src/main/java/com/arquisoft/fichas/domain/itemfichaperfil/aggregate/ItemFichaPerfilAggregate.java` | Agregar método de negocio `modificarContenido(String nuevoContenido)` que valida con Notification Pattern y actualiza solo el campo `contenido` (no modifica `tipoItem` ni `fichaPerfilId`). |
...
- **Métodos principales:**
    - `ejecutar(ModificarItemFichaPerfilCommand command): void` — implementa el `VoidInputPort`. 
      ...
      3. Valida que el estudiante sea propietario de la ficha (`fichaPerfilQueryOutputPort.esEstudiantePropietario(item.getFichaPerfilId(), estudianteId)` — lanza `ItemFichaNoPropiaException` si no es propietario, esa excepción ya existe de HU-031; extiende `ApplicationException` → 400, no 403).
      4. Llama a `item.modificarContenido(command.contenido())` — valida con Notification Pattern (puede lanzar `DomainValidationException`).
      5. Persiste con `itemFichaPerfilOutputPort.guardar(item)`.
      6. Loguea con `log.info(FichasMessages.ItemFichaPerfil.LOG_MODIFICADO, itemId)`.
...
- **Dependencias:** `ModificarItemFichaPerfilInputPort`, `ModificarItemFichaPerfilRequestDTO`, `@RestController`, `@RequestMapping`, `@PutMapping`, `@PathVariable`, `@Valid`, `@RequestBody`, `@AuthenticationPrincipal`, `Jwt`, `ResponseEntity`, `HttpStatus`, `@Slf4j`, `@RequiredArgsConstructor`, anotaciones Swagger.
...
|---|---|---|---|
| `fichas:item-ficha-perfil:update` | `estudiante` | `PUT /fichas-perfil/{itemId}/items/` | Permite al estudiante modificar el contenido de un ítem de su propia ficha de perfil |
...
| `ItemFichaPerfilAggregateTest` | `debeLimpiarEspacios_cuandoModificarContenido` | `modificarContenido("  texto  ")` actualiza a `"texto"` (sin espacios inicio/fin) |
...
| `ModificarItemFichaPerfilUseCaseTest` | `debePropagar Excepcion_cuandoRepositorioFalla` | `itemFichaPerfilOutputPort.guardar(item)` lanza excepción → se propaga sin capturar |
...
| `ModificarItemFichaPerfilInputAdapterTest` | `debe400_cuandoEstudianteNoEsPropietario` | `ItemFichaNoPropiaException` → `400` con `errorCode` (extiende `ApplicationException`, no `403`) |
