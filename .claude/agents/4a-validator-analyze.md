...
| Se declaró un `record` vacío o un centinela `VACIO` solo para rellenar la entrada de una consulta que no la tiene (en vez de `SupplierUseCase`) — `VACIO` representa un dato que pudo estar y no está; aquí no hay dato | ❌ |
...
| Método de un `OutputPort`/`QueryOutputPort`/`CommandOutputAdapter`/`QueryOutputAdapter` nombrado en inglés (`save`, `findById`, `existsById`…) en vez de español (`guardar`, `buscarPorId`, `existePorId`). Excepción: los métodos del `{Entidad}CommandRepository`/`{Entidad}QueryRepository` de Spring Data sí van en inglés — Spring Data deriva la query del nombre | ❌ |
