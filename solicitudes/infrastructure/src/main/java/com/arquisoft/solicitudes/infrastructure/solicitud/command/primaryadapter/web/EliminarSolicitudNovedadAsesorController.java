...
public class EliminarSolicitudNovedadAsesorController {

    private final EliminarSolicitudNovedadAsesorInteractor eliminarSolicitudNovedadAsesorInteractor;

    @DeleteMapping(
            "${rutas.solicitudes.solicitud.novedad-asesor-por-id:/novedad-asesor/{solicitudId}}")
    @PreAuthorize(SolicitudesAuthorities.Expresiones.HAS_SOLICITUD_NOVEDAD_ASESOR_DELETE)
    @Operation(
            summary = SolicitudesApiMessages.Solicitud.ELIMINAR_NOVEDAD_ASESOR_SUMMARY,
            description = SolicitudesApiMessages.Solicitud.ELIMINAR_NOVEDAD_ASESOR_DESCRIPTION,
            security = @SecurityRequirement(name = ApiSecurity.BEARER_AUTH)
    )
    @ApiResponses({
            @ApiResponse(responseCode = ApiCodes.NO_CONTENT,
                    description = SolicitudesApiMessages.Solicitud.ELIMINAR_NOVEDAD_ASESOR_RESP_204),
            @ApiResponse(responseCode = ApiCodes.BAD_REQUEST,
                    description = SolicitudesApiMessages.Solicitud.ELIMINAR_NOVEDAD_ASESOR_RESP_400,
                    content = @Content(schema = @Schema(implementation = ErrorResponseDTO.class))),
            @ApiResponse(responseCode = ApiCodes.UNAUTHORIZED,
                    description = SolicitudesApiMessages.Comun.RESP_401),
            @ApiResponse(responseCode = ApiCodes.FORBIDDEN,
                    description = SolicitudesApiMessages.Solicitud.ELIMINAR_NOVEDAD_ASESOR_RESP_403),
            @ApiResponse(responseCode = ApiCodes.UNPROCESSABLE,
                    description = SolicitudesApiMessages.Solicitud.ELIMINAR_NOVEDAD_ASESOR_RESP_422,
                    content = @Content(schema = @Schema(implementation = ErrorResponseDTO.class)))
    })
    public ResponseEntity<Void> eliminar(@PathVariable String solicitudId,
        ...
        eliminarSolicitudNovedadAsesorInteractor.ejecutar(
                EliminarSolicitudNovedadAsesorCommand.crear(solicitudId, remitenteUsuario));
