...
class AsesorFichaAgregadoConsumerTest {
    ...
    private AsesorFichaAgregadoConsumer consumer;

    @BeforeEach
    void setUp() {
        var gestorTraza = new GestorTrazaImpl(new MdcContextoDiagnosticoOutputAdapter(), false);
        consumer = new AsesorFichaAgregadoConsumer(
    ...
    void debeRegistrarLaReplica_cuandoLlegaElEvento() throws Exception {
        ...
        // Act
        consumer.onAsesorFichaAgregado(mensaje(json), channel);
        ...
        assertThat(captor.getValue().identificador()).isEqualTo("ASE-001");
        assertThat(captor.getValue().nombre()).isEqualTo("Ana Asesora");
