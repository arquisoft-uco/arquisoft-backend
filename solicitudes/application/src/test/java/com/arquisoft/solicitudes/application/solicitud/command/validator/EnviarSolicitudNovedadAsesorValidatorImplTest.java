...
import com.arquisoft.solicitudes.application.solicitud.command.validator.impl.EnviarSolicitudNovedadAsesorValidatorImpl;
...
class EnviarSolicitudNovedadAsesorValidatorImplTest {

    private final EnviarSolicitudNovedadAsesorValidatorImpl validator =
            new EnviarSolicitudNovedadAsesorValidatorImpl();

    private static EnvioSolicitudNovedadAsesorDomain envio() {
    ...
    void debePasar_cuandoRemitenteYDestinatarioExisten() {
        // Act & Assert
        assertThatCode(() -> validator.validarExistenciaUsuarios(
                envio(), usuario(UUID.randomUUID()), usuario(UUID.randomUUID())))
    ...
    void debeLanzarRemitenteNoEncontrado_cuandoElRemitenteNoExiste() {
        ...
        // Act & Assert
        assertThatThrownBy(() -> validator.validarExistenciaUsuarios(
                envio, UsuarioDomain.VACIO, usuario(UUID.randomUUID())))
    ...
    void debeLanzarDestinatarioNoEncontrado_cuandoElDestinatarioNoExiste() {
        ...
        // Act & Assert
        assertThatThrownBy(() -> validator.validarExistenciaUsuarios(
                envio, usuario(UUID.randomUUID()), UsuarioDomain.VACIO))
    ...
    void debeReportarPrimeroLaAusenciaDelRemitente_cuandoAmbosFaltan() {
        ...
        // Act & Assert
        assertThatThrownBy(() -> validator.validarExistenciaUsuarios(
                envio(), UsuarioDomain.VACIO, UsuarioDomain.VACIO))
                .isInstanceOf(RemitenteNoEncontradoException.class);
    }
    ...
    void debeLanzarSolicitudDuplicada_cuandoLaClaveYaExiste() {
        // Arrange
        var clave = new ClaveSolicitud(UUID.randomUUID(), UUID.randomUUID(), Instant.now(), "novedad");
