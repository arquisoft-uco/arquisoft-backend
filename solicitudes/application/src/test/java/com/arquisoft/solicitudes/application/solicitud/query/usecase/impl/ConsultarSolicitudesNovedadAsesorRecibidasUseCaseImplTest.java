...
class ConsultarSolicitudesNovedadAsesorRecibidasUseCaseImplTest {
    ...
    @InjectMocks
    private ConsultarSolicitudesNovedadAsesorRecibidasUseCaseImpl useCase;
    ...
    void debeLoguearEntradaYCierre_conSolicitudKey() {
        ...
        // Assert
        verify(logger).debug(eq(SolicitudKey.LOG_CONSULTANDO_NOVEDAD_ASESOR_RECIBIDAS),
                eq(0), eq(10), eq(criteria.tieneFiltros()), eq(criteria.tieneOrden()));
        verify(logger).debug(eq(SolicitudKey.LOG_CONSULTA_NOVEDAD_ASESOR_RECIBIDAS_COMPLETADA),
                eq(0L), eq(0), eq(10));
    }
}
