...
class ConsultarRespuestasNovedadCoordinadorEnviadasUseCaseImplTest {
    ...
    @InjectMocks
    private ConsultarRespuestasNovedadCoordinadorEnviadasUseCaseImpl useCase;
    ...
    void debeLoguearEntradaYCierre_conRespuestaKey() {
        ...
        // Assert
        verify(logger).debug(eq(RespuestaKey.LOG_CONSULTANDO_NOVEDAD_COORDINADOR_ENVIADAS),
                eq(0), eq(10), eq(criteria.tieneFiltros()), eq(criteria.tieneOrden()));
        verify(logger).debug(eq(RespuestaKey.LOG_CONSULTA_NOVEDAD_COORDINADOR_ENVIADAS_COMPLETADA),
                eq(0L), eq(0), eq(10));
    }
}
