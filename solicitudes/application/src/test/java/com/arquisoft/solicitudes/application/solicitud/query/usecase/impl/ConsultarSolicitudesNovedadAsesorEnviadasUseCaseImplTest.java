...
class ConsultarSolicitudesNovedadAsesorEnviadasUseCaseImplTest {
    ...
    @InjectMocks
    private ConsultarSolicitudesNovedadAsesorEnviadasUseCaseImpl useCase;
    ...
    void debeLoguearEntradaYCierre_conSolicitudKey() {
        ...
        // Assert
        verify(logger).debug(eq(SolicitudKey.LOG_CONSULTANDO_NOVEDAD_ASESOR_ENVIADAS),
                eq(0), eq(10), eq(criteria.tieneFiltros()), eq(criteria.tieneOrden()));
        verify(logger).debug(eq(SolicitudKey.LOG_CONSULTA_NOVEDAD_ASESOR_ENVIADAS_COMPLETADA),
                eq(0L), eq(0), eq(10));
    }
}
