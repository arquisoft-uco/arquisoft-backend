...
class ConsultarRespuestasNovedadAsesorRecibidasUseCaseImplTest {
    ...
    @InjectMocks
    private ConsultarRespuestasNovedadAsesorRecibidasUseCaseImpl useCase;
    ...
    private static RespuestaReadModel respuesta() {
        ...
        var destinatario = new DestinatarioReadModel(
                UUID.randomUUID(), "COORD-1", "Asesora", "coord@uco.edu.co");
        var solicitud = new SolicitudReadModel(UUID.randomUUID(), "una novedad",
                Instant.now(), "NOVEDAD_PARA_EL_ASESOR", "Novedad para el Asesor",
    ...
    void debeLoguearEntradaYCierre_conRespuestaKey() {
        ...
        // Assert
        verify(logger).debug(eq(RespuestaKey.LOG_CONSULTANDO_NOVEDAD_ASESOR_RECIBIDAS),
                eq(0), eq(10), eq(criteria.tieneFiltros()), eq(criteria.tieneOrden()));
        verify(logger).debug(eq(RespuestaKey.LOG_CONSULTA_NOVEDAD_ASESOR_RECIBIDAS_COMPLETADA),
                eq(0L), eq(0), eq(10));
    }
}
