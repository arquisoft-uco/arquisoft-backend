...
class ConsultarAsesoresVigentesUseCaseImplTest {
    ...
    @InjectMocks
    private ConsultarAsesoresVigentesUseCaseImpl useCase;
    ...
    void debeDelegarEnElPuertoYRetornarSuResultado() {
        ...
        verify(asesorQueryOutputPort).consultarVigentes(criteria);
    }
    ...
    void debeRetornarPaginaVacia_cuandoElPuertoNoEncuentraRegistros() {
        // Arrange
        var criteria = AsesorVigenteCriteria.builder().pagina(0).tamanio(10).build();
        when(asesorQueryOutputPort.consultarVigentes(any())).thenReturn(PaginatedResult.of(List.of(), 0, 10, 0L));
    ...
    void debeRegistrarDebugEntradaYCierre_sinInfo() {
        ...
        // Assert
        verify(logger).debug(eq(ConsultarAsesoresVigentesKey.LOG_CONSULTANDO),
                eq(criteria.getPagina()), eq(criteria.getTamanio()), eq(criteria.tieneFiltros()), eq(criteria.tieneOrden()));
        verify(logger).debug(eq(ConsultarAsesoresVigentesKey.LOG_CONSULTA_COMPLETADA),
