...
class CrearUsuarioUseCaseTest {
    ...
    void debeCrearUsuario_cuandoDatosValidos() {
        ...
        // Act
        UUID resultado = crearUsuarioUseCase.ejecutar(usuario);
    ...
    void debePublicarEvento_cuandoEjecucionExitosa() {
        // Arrange
        UsuarioDomain usuario = UsuarioDomain.crear("test@example.com", UsuarioRole.ASESOR_FICHA);
        when(emailUsuarioExisteFinder.obtener(anyString())).thenReturn(false);
    ...
    void debeMapearAggregateAEntity_cuandoGuarda() {
        // Arrange
        UsuarioDomain usuario = UsuarioDomain.crear("admin@example.com", UsuarioRole.ADMINISTRADOR);
