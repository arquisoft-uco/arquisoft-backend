...
class ConsultarCoordinadoresAdministradorUseCaseImplTest {
    ...
    @InjectMocks
    private ConsultarCoordinadoresAdministradorUseCaseImpl useCase;

    @Test
    void debeDelegarEnElPuertoYRetornarSuResultado() {
        // Arrange
        var criteria = CoordinadorCriteria.builder().pagina(0).tamanio(10).build();
        var esperado = PaginatedResult.of(
                List.of(new CoordinadorReadModel(UUID.randomUUID(), "1000", "Ana", "ana@uco.edu.co",
                        "3000000000", "ACTIVO", true)),
                0, 10, 1L);
        when(coordinadorQueryOutputPort.consultarTodos(criteria)).thenReturn(esperado);
        ...
        verify(coordinadorQueryOutputPort).consultarTodos(criteria);
    }
    ...
    void debeRetornarPaginaVacia_cuandoElPuertoNoEncuentraRegistros() {
        // Arrange
        var criteria = CoordinadorCriteria.builder().pagina(0).tamanio(10).build();
        when(coordinadorQueryOutputPort.consultarTodos(any())).thenReturn(PaginatedResult.of(List.of(), 0, 10, 0L));
    ...
    void debeRegistrarDebugEntradaYCierre_sinInfo() {
        ...
        // Assert
        verify(logger).debug(eq(ConsultarCoordinadoresAdministradorKey.LOG_CONSULTANDO),
                eq(criteria.getPagina()), eq(criteria.getTamanio()), eq(criteria.tieneFiltros()), eq(criteria.tieneOrden()));
        verify(logger).debug(eq(ConsultarCoordinadoresAdministradorKey.LOG_CONSULTA_COMPLETADA),
