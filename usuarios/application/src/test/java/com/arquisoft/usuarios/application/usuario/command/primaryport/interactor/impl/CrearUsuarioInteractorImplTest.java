...
class CrearUsuarioInteractorImplTest {
    ...
    void debeDelegarEnElUseCase_cuandoEjecutar() {
        ...
        when(crearUsuarioUseCase.ejecutar(dominioEquivalenteA(command))).thenReturn(idEsperado);
        ...
        verify(crearUsuarioUseCase).ejecutar(dominioEquivalenteA(command));
    }

    // El caso de uso recibe el dominio que construyo el mapper: se compara por valor.
    private static UsuarioDomain dominioEquivalenteA(CrearUsuarioCommand command) {
        return argThat(usuario -> usuario.getEmail().equals(command.email())
                && usuario.getRol() == command.rol());
    }
}
