...
import com.arquisoft.usuarios.infrastructure.estudiante.command.primaryadapter.web.mapper.RemoverEstudianteRequestMapper;
...
public class RemoverEstudianteController {

    private final RemoverEstudianteInteractor removerEstudianteInteractor;

    @DeleteMapping("${rutas.usuarios.usuarios.estudiante:/{usuarioId}/estudiante}")
    @PreAuthorize(UsuariosAuthorities.Expresiones.HAS_ESTUDIANTE_DELETE)
    @Operation(
            summary = UsuariosApiMessages.Estudiante.REMOVER_SUMMARY,
            description = UsuariosApiMessages.Estudiante.REMOVER_DESCRIPTION,
            security = @SecurityRequirement(name = ApiSecurity.BEARER_AUTH)
    )
    @ApiResponses({
            @ApiResponse(responseCode = ApiCodes.NO_CONTENT,
                    description = UsuariosApiMessages.Estudiante.REMOVER_RESP_204),
            @ApiResponse(responseCode = ApiCodes.BAD_REQUEST,
                    description = UsuariosApiMessages.Estudiante.REMOVER_RESP_400,
            ...
            @ApiResponse(responseCode = ApiCodes.UNPROCESSABLE,
                    description = UsuariosApiMessages.Estudiante.REMOVER_RESP_422,
                    content = @Content(schema = @Schema(implementation = ErrorResponseDTO.class))),
            @ApiResponse(responseCode = ApiCodes.SERVICE_UNAVAILABLE,
                    description = UsuariosApiMessages.Estudiante.REMOVER_RESP_503,
                    content = @Content(schema = @Schema(implementation = ErrorResponseDTO.class)))
    })
    public ResponseEntity<Void> remover(@PathVariable UUID usuarioId) {
        removerEstudianteInteractor.ejecutar(RemoverEstudianteRequestMapper.toCommand(usuarioId));
