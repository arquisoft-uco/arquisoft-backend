...
import com.arquisoft.usuarios.infrastructure.coordinador.query.primaryadapter.web.mapper.ConsultarCoordinadoresAdministradorRequestMapper;
import com.arquisoft.usuarios.infrastructure.coordinador.query.primaryadapter.web.mapper.CoordinadorResponseMapper;
...
public class ConsultarCoordinadoresAdministradorController {

    private final ConsultarCoordinadoresAdministradorInteractor consultarCoordinadoresAdministradorInteractor;

    @PostMapping("${rutas.usuarios.usuarios.coordinadores-administrador:/coordinadores/administrador}")
    @PreAuthorize(UsuariosAuthorities.Expresiones.HAS_COORDINADOR_ADMINISTRADOR_VIEW)
    @Operation(
            summary = UsuariosApiMessages.Coordinador.CONSULTAR_ADMINISTRADOR_SUMMARY,
            description = UsuariosApiMessages.Coordinador.CONSULTAR_ADMINISTRADOR_DESCRIPTION,
    ...
    public ResponseEntity<PageResponseDTO<CoordinadorResponseDTO>> consultar(
            @RequestBody(required = false) QueryCriteriaRequestDTO request) {

        var resultado = consultarCoordinadoresAdministradorInteractor.ejecutar(
                ConsultarCoordinadoresAdministradorRequestMapper.toQuery(request));

        return ResponseEntity.ok(PageResponseDTO.from(
                resultado.map(CoordinadorResponseMapper::toResponse)));
    }
}
