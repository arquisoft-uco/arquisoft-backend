...
@WebMvcTest(ConsultarAsesoresVigentesController.class)
@Import({AppLoggerConfig.class, GlobalAppExceptionHandler.class, TrazabilidadConfig.class,
        ConsultarAsesoresVigentesControllerTest.TestSecurityConfig.class})
class ConsultarAsesoresVigentesControllerTest {

    private static final String RUTA = "/usuarios/asesores/vigentes";
    ...
    @MockitoBean
    private ConsultarAsesoresVigentesInteractor consultarAsesoresVigentesInteractor;

    private SecurityMockMvcRequestPostProcessors.JwtRequestPostProcessor vigente() {
        return SecurityMockMvcRequestPostProcessors.jwt()
                .authorities(new SimpleGrantedAuthority(UsuariosAuthorities.ASESOR_VIGENTE_VIEW));
    }

    @Test
    void debe200ConEstadoSinVigente_cuandoBodyVacio() throws Exception {
        // Arrange
        var readModel = new AsesorVigenteReadModel(UUID.randomUUID(), "1001", "Ana Ramirez",
                "ana.ramirez@uco.edu.co", "3000000000", "ACTIVO");
        when(consultarAsesoresVigentesInteractor.ejecutar(any(ConsultaCriteriaQuery.class)))
    ...
    void debe200_cuandoFiltraPorEstado() throws Exception {
        ...
        // Assert
        verify(consultarAsesoresVigentesInteractor).ejecutar(captor.capture());
        assertThat(captor.getValue().raiz())
                .isEqualTo(NodoFiltro.predicado("estado", FiltroOperador.ES, "ACTIVO"));
    }
