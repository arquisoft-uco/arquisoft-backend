...
@WebMvcTest(RemoverCoordinadorController.class)
@Import({AppLoggerConfig.class, GlobalAppExceptionHandler.class, TrazabilidadConfig.class,
        RemoverCoordinadorControllerTest.TestSecurityConfig.class})
class RemoverCoordinadorControllerTest {

    private static final String RUTA = "/usuarios/{usuarioId}/coordinador";
    ...
    void debe204YDelegarConElUsuario_cuandoPeticionValida() throws Exception {
        ...
        verify(removerCoordinadorInteractor).ejecutar(new RemoverCoordinadorCommand(usuario));
    }
    ...
    void debe400SinInvocarInteractor_cuandoUsuarioIdNoEsUuid() throws Exception {
        ...
        verify(removerCoordinadorInteractor, never()).ejecutar(any());
    }
    ...
    @Test
    void debe422ConSuCodigo_cuandoCoordinadorNoEstaVigente() throws Exception {
        // Arrange
        var usuario = UtilUUID.generarNuevoUUID();
        doThrow(new CoordinadorNoEncontradoException(usuario)).when(removerCoordinadorInteractor).ejecutar(any());

        // Act & Assert
        mockMvc.perform(delete(RUTA, usuario).with(administrador()))
                .andExpect(status().isUnprocessableEntity())
                .andExpect(jsonPath("$.errorCode").value(UsuariosCodes.Coordinador.COORDINADOR_NO_ENCONTRADO));
    }
