...
class EstudianteAgregadoProyectosConsumerTest {

    @Mock
    private AgregarEstudianteProyectosInteractor agregarEstudianteProyectosInteractor;
    ...
    private EstudianteAgregadoProyectosConsumer adapter;

    @BeforeEach
    void setUp() {
        adapter = new EstudianteAgregadoProyectosConsumer(
                agregarEstudianteProyectosInteractor,
                new ObjectMapper(),
                logger,
                new GestorTrazaImpl(new MdcContextoDiagnosticoOutputAdapter(), false));

        lenient().when(agregarEstudianteProyectosInteractor.ejecutar(any()))
    ...
    void debeInvocarElInteractor_cuandoLlegaElEvento() throws Exception {
        ...
        // Assert
        verify(agregarEstudianteProyectosInteractor).ejecutar(any());
    }
    ...
    void debeEnviarNackSinReencolar_cuandoElPayloadEsEnvenenado() throws Exception {
        ...
        doThrow(new IllegalArgumentException("id invalido"))
                .when(agregarEstudianteProyectosInteractor).ejecutar(any());
    ...
    void debeConfirmarElMensaje_cuandoElResultadoEsDuplicada() throws Exception {
        // Arrange
        when(agregarEstudianteProyectosInteractor.ejecutar(any()))
                .thenReturn(new AgregacionEstudianteResult.Duplicada(UUID.randomUUID()));
    ...
    void debeConfirmarElMensaje_cuandoElResultadoEsDescartada() throws Exception {
        // Arrange
        when(agregarEstudianteProyectosInteractor.ejecutar(any())).thenReturn(
                new AgregacionEstudianteResult.Descartada(UUID.randomUUID(), Instant.now()));

        // Act
        adapter.onEstudianteAgregado(mensajeCon(UUID.randomUUID().toString(), 4L), channel);
