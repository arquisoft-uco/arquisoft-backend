...
@DataJpaTest
class AsesorProyectosCommandOutputAdapterTest {

    @Autowired
    private AsesorProyectosCommandRepository asesorCommandRepository;
    ...
    void debeGuardarAsesor_cuandoSePersiste() {
        // Arrange
        var adapter = new AsesorProyectosCommandOutputAdapter(asesorCommandRepository, logger);
        var id = UUID.randomUUID();
        var ocurridoEn = Instant.now();

        // Act
        adapter.guardar(new AsesorEntity(id, "20161020123", "Ana Perez", "ana@uco.edu.co", ocurridoEn));
