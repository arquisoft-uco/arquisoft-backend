...
class CoordinadorAgregadoProyectosConsumerTest {

    @Mock
    private AgregarCoordinadorProyectosInteractor agregarCoordinadorProyectosInteractor;
    ...
    private CoordinadorAgregadoProyectosConsumer adapter;

    @BeforeEach
    void setUp() {
        adapter = new CoordinadorAgregadoProyectosConsumer(
                agregarCoordinadorProyectosInteractor,
                new ObjectMapper(),
                logger,
                new GestorTrazaImpl(new MdcContextoDiagnosticoOutputAdapter(), false));

        lenient().when(agregarCoordinadorProyectosInteractor.ejecutar(any()))
    ...
    void debeInvocarElInteractor_cuandoLlegaElEvento() throws Exception {
        ...
        // Assert
        verify(agregarCoordinadorProyectosInteractor).ejecutar(any());
    }
    ...
    void debeEnviarNackSinReencolar_cuandoElPayloadEsEnvenenado() throws Exception {
        ...
        doThrow(new IllegalArgumentException("id invalido"))
                .when(agregarCoordinadorProyectosInteractor).ejecutar(any());
    ...
    void debeConfirmarElMensaje_cuandoElResultadoEsDescartada() throws Exception {
        // Arrange
        when(agregarCoordinadorProyectosInteractor.ejecutar(any())).thenReturn(
