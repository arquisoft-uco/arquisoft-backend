...
class AsesorRemovidoPayloadTest {

    private final tools.jackson.databind.json.JsonMapper mapper =
            new RabbitMQConfig().rabbitObjectMapper();

    private static final class AsesorRemovidoEventoDePrueba extends DomainEvent {
        ...
        private AsesorRemovidoEventoDePrueba(UUID usuario, String identificador, String nombre, String email) {
            super(EventTopics.Usuarios.ASESOR_REMOVIDO, "AsesorRemovidoEvent");
    ...
    void debeConservarTodosLosCampos_cuandoSeLeeElEventoPublicado() {
        // Arrange
        var evento = new AsesorRemovidoEventoDePrueba(
                UtilUUID.generarNuevoUUID(), "1036950123", "Carlos Rios", "carlos@uco.edu.co");

        // Act
        var payload = mapper.readValue(mapper.writeValueAsString(evento), AsesorRemovidoPayload.class);
    ...
    void debeDeserializarConNull_cuandoFaltaUnCampo() {
        ...
        var json = """
                {"idEvento":"evt-1","usuario":"11111111-1111-1111-1111-111111111111",
                 "identificador":"1036950123","nombre":"Carlos Rios"}
                """;

        // Act
        var payload = mapper.readValue(json, AsesorRemovidoPayload.class);
