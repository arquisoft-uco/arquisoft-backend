...
class AsesorAgregadoProyectosConsumerTest {

    @Mock
    private AgregarAsesorProyectosInteractor agregarAsesorProyectosInteractor;
    ...
    private AsesorAgregadoProyectosConsumer adapter;

    @BeforeEach
    void setUp() {
        adapter = new AsesorAgregadoProyectosConsumer(
                agregarAsesorProyectosInteractor,
                new ObjectMapper(),
                logger,
                new GestorTrazaImpl(new MdcContextoDiagnosticoOutputAdapter(), false));

        lenient().when(agregarAsesorProyectosInteractor.ejecutar(any()))
    ...
    void debeInvocarElInteractor_cuandoLlegaElEvento() throws Exception {
        ...
        // Assert
        verify(agregarAsesorProyectosInteractor).ejecutar(any());
    }
    ...
    void debeEnviarNackSinReencolar_cuandoElPayloadEsEnvenenado() throws Exception {
        ...
        doThrow(new IllegalArgumentException("id invalido"))
                .when(agregarAsesorProyectosInteractor).ejecutar(any());
    ...
    void debeConfirmarElMensaje_cuandoElResultadoEsDescartada() throws Exception {
        // Arrange
        when(agregarAsesorProyectosInteractor.ejecutar(any())).thenReturn(
