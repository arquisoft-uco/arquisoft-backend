## Descripción

<!-- Breve descripción de los cambios realizados -->

## Historia / Incidente Relacionado

- [ ] HU-XXX / HT-XXX / INC-XXX: (descripción)

## Tipo de Cambio

- [ ] feat: Nueva funcionalidad
- [ ] fix: Corrección de error
- [ ] refactor: Refactorización sin cambio funcional
- [ ] docs: Cambios en documentación
- [ ] style: Cambios de formato/estilo
- [ ] test: Agregar o modificar tests
- [ ] chore: Tareas de mantenimiento

## Checklist de Revisión

### Convenciones
- [ ] Nomenclatura en español para términos de negocio
- [ ] Sufijos técnicos en inglés (UseCase, Port, Adapter, Controller, DTO)
- [ ] Arquitectura hexagonal respetada (domain → application → infrastructure)
- [ ] Commits siguen Conventional Commits en español

### Calidad
- [ ] Tests unitarios agregados/actualizados (JUnit 6 + Mockito + AssertJ)
- [ ] Cobertura ≥ 75% (JaCoCo)
- [ ] Build exitoso (`./gradlew build`)
- [ ] Sin código muerto ni imports no utilizados

### Funcional
- [ ] Criterios de aceptación de la historia verificados
- [ ] Sin regresiones en funcionalidad existente
- [ ] Endpoints documentados (si aplica)

## Notas para el Reviewer

<!-- Cualquier contexto adicional para facilitar la revisión -->
